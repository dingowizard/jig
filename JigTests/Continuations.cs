...
public class Continuations
{
    ...
    public void ApplyingContinuation() {
        ...
        string result1 = "";
        Continuation.OneArgDelegate setResult1 = (x) => {result1 = x.Print(); return new Continuation.MaybeThunk.None();};
        Expr? x = Jig.Reader.Reader.ReadSyntax(InputPort.FromString("(define cont #f)"));
        Assert.IsNotNull(x);
        Program.Eval(setResult1, x, env);
        string result2 = "";
        Continuation.OneArgDelegate setResult2 = (x) => {result2 = x.Print(); return new Continuation.MaybeThunk.None();};
        ...
        string result3 = "";
        Continuation.OneArgDelegate setResult3 = (x) => {result3 = x.Print(); return new Continuation.MaybeThunk.None();};
