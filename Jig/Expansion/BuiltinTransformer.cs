...
public class BuiltinTransformer  : Transformer {
    ...
    public static Syntax quasiquote(Syntax stx) {
        ...
        if (argE is List.Empty ||
            argE is not IPair) {
            return new Syntax(SyntaxList.FromParams(new Syntax.Identifier(new Form.Symbol("quote")),
                                                      arg),
                                stx.SrcLoc);
            ...
            if (Form.IsKeyword("unquote", arg)) { // (quasiquote (unquote x))
                ...
                return new Syntax(SyntaxList.FromParams(new Syntax.Identifier(new Form.Symbol("cons")),
                ...
                if (Form.IsKeyword("unquote-splicing", stxListArg.ElementAt<Syntax>(0))) {
                    ...
                    // }
                    return new Syntax(SyntaxList.FromParams(new Syntax.Identifier(new Form.Symbol("append")),
                                                              listToSpliceStx,
                                                              new Syntax(SyntaxList.FromParams(new Syntax.Identifier(new Form.Symbol("quasiquote")),
                                                                                           new Syntax(SyntaxList.FromIEnumerable(stxListArg.Skip<Syntax>(1)))),
                                                                     new SrcLoc())),
